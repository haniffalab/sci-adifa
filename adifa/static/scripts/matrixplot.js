...
(function ($) {
  $.fn.matrixplot = function (options) {
    ...
    // private variables
    const widthParent = $("#" + settings.containerId)
      .parent()
      .width();
    ...
    const render = function () {
      ...
      if (varList.length) {
        varList.forEach(function (v) {
          if ($("button[data-gene='" + escapeSelector(v) + "']").length) {
            $("button[data-gene='" + escapeSelector(v) + "']").addClass(
              "active"
            );
          } else {
      ...
      let myGroups;
      if (
        $('div[data-name="' + escapeSelector(colorScaleKey) + '"]').data(
          "type"
        ) === "continuous"
      ) {
      ...
      const groupedValues = Object.fromEntries(
        Object.entries(active.data.values_df).map(([k, v]) => [
          [k],
          Object.fromEntries(myGroups.map((g) => [[g], v[g]])),
        ])
      );

      let svg = d3.select("#canvas_plot").append("svg");
      ...
      // Build X scales and axis:
      const xAxis = svg.append("g").call(d3.axisBottom(x).tickSizeOuter(0));
      ...
      // Build X scales and axis:
      x = d3.scaleBand().range([0, width]).domain(myVars).padding(0.02);
      ...
      // Build X scales and axis:
      y = d3.scaleBand().range([height, 0]).domain(myGroups).padding(0.02);
      ...
      const myColor = d3
        .scaleSequential()
        .domain([active.data.min_value, active.data.max_value])
        .interpolator(interpolator);

      let uid = 0;
      svg
        .selectAll()
        .data(Object.entries(groupedValues), function (d) {
          return d[1];
        })
        .enter()
        .selectAll()
      ...
      d3.select("#canvas-zoom-plus").on("click", function () {
        svg.transition().call(zoom.scaleBy, 2);
      });

      d3.select("#canvas-zoom-minus").on("click", function () {
        svg.transition().call(zoom.scaleBy, 0.5);
      });

      d3.select("#canvas-zoom-reset").on("click", function () {
        svg.call(
          zoom.transform,
          d3.zoomIdentity.translate(widthParent / 2 - width / 2, 100).scale(1)
        );
      });
      endLoader();
    };
    ...
    this.initialize = function () {
                ...
                $("<div/>")
                  ...
                  .append(
                    $("<select/>")
                      ...
                      .append(
                        $("<option/>").attr("value", "Turbo").text("Turbo")
                      )
                      .append(
                        $("<option/>").attr("value", "Viridis").text("Viridis")
                      )
                      .append(
                        $("<option/>").attr("value", "Inferno").text("Inferno")
                      )
                      .append(
                        $("<option/>").attr("value", "Magma").text("Magma")
                      )
                      .append(
                        $("<option/>").attr("value", "Cividis").text("Cividis")
                      )
                      .append($("<option/>").attr("value", "Warm").text("Warm"))
                      .append($("<option/>").attr("value", "Cool").text("Cool"))
                  )
              )
              .append(
                $("<div/>")
      ...
      $.when(doAjax(API_SERVER + "api/v1/datasets/" + datasetId)).then(
        function (d) {
          ...
          if (varList.length) {
            Cookies.set(
              "ds" + datasetId + "-var-list",
              JSON.stringify(varList),
              {
                expires: 30,
                sameSite: "Strict",
                path: window.location.pathname,
    ...
    this.genes = function (el) {
      ...
      if ($(el).hasClass("active")) {
        ...
        varList = varList.filter(function (e) {
          return e !== $(el).data("gene");
        });
      } else {
        const gene = $(el).text();
